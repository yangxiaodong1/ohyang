...
class MatchApply(object):
    ...
    def get_nearest(self, one_user_id, another_user_id):
        apply = self.apply
        query = apply.get_query()
        query = apply.get_by_one_user(query, one_user_id)
        query = apply.get_by_another_user(query, another_user_id)
        query = apply.order_by_id_desc(query)
        return apply.first(query)
    ...
    def add(self, one_user_id, another_user_id, data_dict):
        ...
        if not apply:
            ...
            if success:
                OHHOLog.print_log(ADD_MATCH_APPLY_SUCCESS)
                return self.get(one_user_id, another_user_id)
            else:
                OHHOLog.print_log(ADD_MATCH_APPLY_FAILED)
        else:
            OHHOLog.print_log(VALID_MATCH_APPLY_EXIST)
        return None
...
if __name__ == "__main__":
    MatchApply.add(1, 2, {"match_condition_id": 3})
